...
exports.uploadVideo = async (req, res) => {
  ...
  upload(req, res, async (err) => {
    ...
    try {
      ...
      const { teacherId, syllabus, title, description, course, assignment } =
        req.body;
...
exports.assignToUser = async (req, res) => {
  try {
    const { teacherId, assignToStudent, assignmentId, dueDate, status } =
      req.body;
    if (
      !teacherId ||
      !assignToStudent ||
...
exports.getAssignmentById = async (req, res) => {
  try {
    let data = await Assign.findOne({ _id: req.params.id }).populate(
      "assignToStudent",
      "name email"
    );
    ...
    res.status(500).json({ code: 500, message: "Error in getting details" });
  }
};
